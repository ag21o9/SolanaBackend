...
function normalizePropertyType(value: unknown): 'Residential' | 'Commercial' | 'Industrial' | 'MixedUse' | null {
    ...
    if (normalized === 'mixeduse' || normalized === 'mixed_use' || normalized === 'mixed-use') return 'MixedUse'
...
async function uploadPropertyMetadata(params: {
    ...
    city?: string
    tokenModel: 'nft' | 'spl'
    propertyType: 'Residential' | 'Commercial' | 'Industrial' | 'MixedUse'
}): Promise<string> {
    const metadata = {
        name: params.name,
...
function getMediaFromStep3(step3Data: unknown): { images: string[]; coverImageUrl?: string } {
    if (!step3Data || typeof step3Data !== 'object') {
        return { images: [] }
    }

    const step3 = step3Data as Record<string, unknown>
    const imagesFromImagesKey = Array.isArray(step3.images)
        ? step3.images.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
        : []

    const imagesFromGalleryKey = Array.isArray(step3.gallery)
        ? step3.gallery.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
        : []
...
function toBooleanQuery(value: string | undefined): boolean | undefined {
...
function getDraftWorkflowStatus(draft: { submittedAt: Date | null; property: { status: string } | null }): 'not_submitted' | 'submitted' | 'approved' {
    if (!draft.submittedAt) return 'not_submitted'
    if (draft.property) return 'approved'
    return 'submitted'
}
...
propertyRouter.get('/listings/drafts', async (req, res) => {
    try {
        ...
        const { submitted, approved, stepCompletedMin, stepCompletedMax, page, limit } = req.query as {
            submitted?: string
            approved?: string
            stepCompletedMin?: string
            stepCompletedMax?: string
            page?: string
            limit?: string
        }
        ...
        const where: Record<string, unknown> = { userWallet }
        const stepCompletedFilter: Record<string, number> = {}
        ...
        if (submittedBool === true) where.submittedAt = { not: null }
        if (submittedBool === false) where.submittedAt = null

        if (approvedBool === true) where.propertyId = { not: null }
        if (approvedBool === false) where.propertyId = null
        ...
        const [drafts, total] = await Promise.all([
            ...
            prisma.listingDraft.count({ where: where as any }),
        ])
...
propertyRouter.get('/properties', async (req, res) => {
    try {
        const { status, country, city, type, page, limit } = req.query as {
            status?: string
            country?: string
            city?: string
            type?: string
...
propertyRouter.post('/listings/draft', async (req, res) => {
    try {
        ...
        const { step, basicInfo } = req.body as {
            step?: number
            basicInfo?: unknown
        }
...
propertyRouter.patch('/listings/:draftId', async (req, res) => {
    try {
        ...
        const { draftId } = req.params
        const { step, tokenomics, mediaUploads, legalDocs } = req.body as {
            step?: number
            tokenomics?: unknown
...
propertyRouter.post('/mint/property', async (req, res) => {
    try {
        ...
        if (city !== undefined) propertyData.city = city
        if (addressFull !== undefined) propertyData.addressFull = addressFull
